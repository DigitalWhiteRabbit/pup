...
tools/tg-service/
├── CLAUDE.md                    # этот файл
├── USER-GUIDE.md                # руководство пользователя (README как такового нет)
...
│   │   ├── __init__.py
│   │   ├── anthropic_client.py
│   │   └── (embeddings для RAG живут в KB-слое, отдельного embeddings.py нет)
│   │
...
│   │                            # (отдельного login.html нет — auth по x-admin-token)
│
├── tests/
│   ├── __init__.py
│   ├── conftest.py
...
| Stage       | Что                                                                    | Статус       |
| ----------- | ---------------------------------------------------------------------- | ------------ |
| **STAGE 0** | Инфраструктура: FastAPI + SQLite + Celery + Redis + AES-256 + PM2      | ✅ Готово    |
| **STAGE 1** | Аккаунты + Прокси + Конвертер + Прогрев                                | ✅ Готово¹   |
| **STAGE 2** | Парсер + Базы аудитории + Каналы + Чекер + Дашборд                     | ✅ Готово    |
| **STAGE 3** | AI Промоутер + AI Продажник + Нейрокомментинг + Автоответчик + Шаблоны | ⚠️ Частично² |
| **STAGE 4** | Рассылка в ЛС + Рассылка по чатам + Инвайтинг                          | ✅ Готово    |
| **STAGE 5** | Накрутка + Масслайкинг + Клонер + Создание чатов + Настройки           | ✅ Готово    |

¹ Прогрев реализован, но live-тест на реальных аккаунтах не прогонялся (DEBT 2.4).
² Промоутер / Автоответчик / Шаблоны — готовы. **AI Продажник + RAG** (DEBT 3.3) и
**Нейрокомментинг** (`commenting.py`, placeholder-dispatch, DEBT 3.4) — воркеры не дописаны.

> Полный статус долгов — `DEBT-PLAN.md`. Открытые баги/хвосты — `TODO.md`.
> Ревью кода и приоритеты фиксов — `CODE_REVIEW.md`.
> Proxy-Seller вырезан 2026-05-26 — не возвращать.
...
**Версия:** v1.1 · 1 июня 2026 (актуализировано после миграции на новый Mac)
**Автор:** Bruno + Claude (через ПУП)

> Состояние на 2026-06-01: все 5 стадий собраны (см. roadmap выше; STAGE 3 частично).
> 31 API-роутер подключён в `app/main.py`, 46 таблиц в `schema.sql`, ~31k строк Python.
> При миграции снапшот `wip/migration` имел битый `main.py` (импорт удалённого
> proxy_seller + не подключены 7 роутеров) — исправлено, см. `CODE_REVIEW.md` (C1).
