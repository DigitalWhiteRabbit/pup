...
const DB_PATH = path.join(__dirname, "..", "data", "parser.db");
...
// SQLite не поддерживает ADD COLUMN IF NOT EXISTS, проверяем через PRAGMA.
...
if (!columnExists("dialogues", "message_count")) {
  safeExec(`ALTER TABLE dialogues ADD COLUMN message_count INTEGER DEFAULT 0`);
}
...
if (!columnExists("leads", "followup_attempts")) {
  safeExec(`ALTER TABLE leads ADD COLUMN followup_attempts INTEGER DEFAULT 0`);
}
...
// Seed: дефолтные red_flags для проекта CopyBanner (id=3)
try {
  const _proj = db
  ...
  if (_proj && _proj.content_red_flags == null) {
    ...
    console.log("[migrate] content_red_flags seed: project id=3");
  }
} catch (e) {
  console.error("[migrate] content_red_flags seed:", e.message);
}
...
safeExec(`CREATE INDEX IF NOT EXISTS idx_leads_locked ON leads(locked_until)`);

// Daily-cap counters table (sent_today_email / sent_today_tg / date)
...
// Key-value настройки (runtime-переключаемые)
...
// Очередь "Review mode" — reply, которые ожидают подтверждения админом
...
// ─── lead_emails: быстрый exact-match по email из inbox ────────────
...
// Population: если таблица пуста, заполнить из leads.email (idempotent: INSERT OR IGNORE)
try {
  ...
  if (cnt && cnt.n === 0) {
    ...
    if (rows.length > 0) {
      ...
      tx();
      console.log(`[migrate] populated lead_emails from ${rows.length} leads`);
    }
  }
} catch (e) {
  console.error("[migrate] lead_emails populate failed:", e.message);
}

// ─── messages.resend_id: индексируемая колонка вместо LIKE по metadata ──
...
// Backfill resend_id для существующих сообщений
try {
  db.exec(`UPDATE messages SET resend_id = json_extract(metadata, '$.resend_id')
           WHERE metadata LIKE '%resend_id%' AND resend_id IS NULL`);
} catch (e) {
  console.error("[migrate] resend_id backfill failed:", e.message);
}
// Триггер: автоматически выставлять resend_id при INSERT messages
...
// ─── Dev tasks tracker (внутренний, для разработки AI Agent for Bruno) ──
...
const stmts = {
  ...
  // Pick next ready lead for outreach — учитывает locked_until
  ...
  // Pick leads with new replies that need a response
  pickLeadsWithNewReplies: db.prepare(`
    ...
    WHERE l.lead_status IN ('ready','in_work')
      ...
      -- Не регенерировать reply, если админ уже отклонил вариант после последнего входящего
      AND NOT EXISTS (
  ...
  // Follow-up: лиды где последний out старше @cutoff и нет in после out (фильтр по времени теперь в SQL)
  ...
  // lead_emails: быстрый exact-match
...
// Helper: пересинхронизировать lead_emails при изменении email лида.
function syncLeadEmails(leadId, emailField) {
...
module.exports = { db, stmts, syncLeadEmails };
