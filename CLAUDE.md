...
- Для аудита и разработки Claude Code **ОБЯЗАН** использовать сабагентов (Task) для аудита/ревью и применять доступные Claude-скиллы (`security-review` и др.), где это релевантно. **Любая правка проходит сабагент-проверку перед коммитом.**

_(EN) For audits, research, and development you MUST use subagents (Task agents): split the work and run them IN PARALLEL (fan-out), then synthesize. Don't do large reviews linearly on your own. Use available skills where relevant (`security-review` for security audits, `review` for change review, etc.). Do large tasks thoroughly and in multiple passes, not superficially. For audit and development in this project Claude Code MUST use subagents (Task) for audit/review and apply available Claude skills (`security-review` etc.) where relevant. Every change passes a subagent review before commit._
